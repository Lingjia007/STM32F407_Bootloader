...
static tuz_force_inline
    tuz_BOOL
    _cache_success_finish(const _TInputCache *self) { return (self->cache_end != 0); }
...
static tuz_try_inline
    tuz_fast_uint8
    _cache_read_1byte(struct _TInputCache *self)
{
...
#endif
    static tuz_length_t
    _cache_unpack_pos_len(tuz_TStream *self)
{
...
tuz_TResult tuz_TStream_decompress_partial(tuz_TStream *self, tuz_byte *cur_out_data, tuz_size_t *data_size)
{
    ...
    copyDict_cmp_process:
        if (self->_state.dictType_len)
        { // copy from dict or out_data
            // copyDict_process:
    ...
    type_process:
    {
        if (_cache_read_1bit(self) == tuz_codeType_dict)
        {
            ...
            else
            {
                ...
                if (saved_dict_pos > tuz_kBigPosForLen)
                    ++saved_len;
            }
            self->_state.isHaveData_back = tuz_FALSE;
...
#ifdef __RUN_MEM_SAFE_CHECK
                if (saved_dict_pos >= self->_dict.dict_size)
                    return tuz_DICT_POS_ERROR;
#endif
            ...
            { // ctrlType
                ...
                self->_state.dict_pos_back = 1;
                self->_state.type_count = 0;
                if (tuz_ctrlType_clipEnd == saved_len)
                { // clip end
                    goto type_process;
                }
                else if (tuz_ctrlType_streamEnd == saved_len)
                { // stream end
                ...
                {
                    return _cache_success_finish(&self->_code_cache) ? tuz_CTRLTYPE_UNKNOW_ERROR : tuz_READ_CODE_ERROR;
                }
            }
        }
        else
...
#endif
    static tuz_length_t
    _mem_unpack_pos_len(_mem_TStream *self)
{
...
#ifdef __RUN_MEM_SAFE_CHECK
                if (saved_dict_pos > (tuz_size_t)(cur_out_data - out_data))
                    return tuz_DICT_POS_ERROR;
                if (dictType_len > (tuz_size_t)(out_data_end - cur_out_data))
...
#ifdef __RUN_MEM_SAFE_CHECK
                    if (literalType_len > (tuz_size_t)(self.in_code_end - src))
                        return tuz_READ_CODE_ERROR;
                    if (literalType_len > (tuz_size_t)(out_data_end - cur_out_data))
                        return tuz_OUT_SIZE_OR_CODE_ERROR;
#endif
