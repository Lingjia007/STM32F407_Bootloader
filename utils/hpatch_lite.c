...
static hpi_force_inline
    hpi_BOOL
    _hpatchi_listener_write_page(hpatchi_listener_extra_t *self, const hpi_byte *data, hpi_size_t data_size)
{
    ...
    while (data_size)
    {
        ...
        if (cur_len == 0)
        { // need out cur page
            if (!_hpatchi_listener_page_out(self))
                return _hpi_FALSE;
...
static hpi_force_inline
    hpi_BOOL
    _hpatchi_listener_extra_flush(hpatchi_listener_extra_t *self)
{
...
static hpi_inline void _hpatchi_listener_extra_init(hpatchi_listener_extra_t *self, hpatchi_listener_t *wrap_listener,
